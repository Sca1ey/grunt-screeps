  ...
  grunt.registerMultiTask(
    ...
    function () {
      ...
      this.files.forEach(function (f) {
        ...
        if (options.ptr) {
          reqParams.path = "/ptr/api/user/code";
        } else if (options.season) {
          reqParams.path = "/season/api/user/code";
        } else {
          reqParams.path = "/api/user/code";
        }
        ...
        var proto = server.http ? http : https,
          req = proto.request(reqParams, function (res) {
            ...
            if (res.statusCode < 200 || res.statusCode >= 300) {
              grunt.fail.fatal(
                "Screeps server returned error code " + res.statusCode
              );
            }
            ...
            res.on("end", function () {
              ...
              try {
                ...
                grunt.log.error(
                  "Error while processing " + serverText + " json: " + e.message
                );
              }
              done();
            });
